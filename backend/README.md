...
.NET 6.0 (or newer): Required to run the backend API.
SQLite: Lightweight database for storing task data.
Node.js & NPM (optional, only if you plan to make modifications to the frontend).
Backend Setup
...
Copy
Edit
git clone https://github.com/yourusername/developer-productivity-app.git
cd developer-productivity-app
...
The backend uses SQLite to store task data. The database will automatically be created when the app runs for the first time. If you need to update the database schema, you can apply migrations using:
...
The frontend will make API requests to http://localhost:5000 to interact with the backend.

Running the App
Start the Backend: Ensure that your .NET Core API is running by following the backend setup instructions.
Start the Frontend: Open the index.html file in your browser to start interacting with the app.
